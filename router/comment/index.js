module.exports = app =>{
    ...
    app.get(`/admin/api/commment/song/:tid`,async (req,res) =>{
        const model = await SongComment.find(req.params.id,req.body).populate('user_id')
        res.send(model)
    })
    ...
    app.get(`/admin/api/commment/mv/:tid`,async (req,res) =>{
        const model = await MvComment.find(req.params.id,req.body).populate('user_id')
        res.send(model)
    })
    ...
    app.get(`/admin/api/commment/album/:tid`,async (req,res) =>{
        const model = await AlbumComment.find(req.params.id,req.body).populate('user_id')
        res.send(model)
    })
